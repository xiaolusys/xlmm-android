package com.jimei.xiaolumeimei.okhttp;
...
public class OkHttpClientManager {
    ...
    mOkHttpClient.newCall(request).enqueue(new Callback() {
      ...
      @Override public void onResponse(final Response response) {
        if (response.code() >= 400 && response.code() <= 599) {
          try {
            sendFailResultCallback(request,
                new RuntimeException(response.body().string()), resCallBack);
          } catch (IOException e) {
            e.printStackTrace();
          }
          return;
        ...
        try {
          ...
          if (resCallBack.mType == String.class) {
            sendSuccessResultCallback(response, string, resCallBack);
          } else {
            Object o = mGson.fromJson(string, resCallBack.mType);
            sendSuccessResultCallback(response, o, resCallBack);
          }
        } catch (IOException e) {
  ...
  public void sendFailResultCallback(final Request request, final Exception e,
  ...
  public void sendSuccessResultCallback(final Response response, final Object object,
      final OkHttpCallback callback) {
    mDelivery.post(new Runnable() {
      @Override public void run() {
  ...
  public void setCertificates(InputStream[] certificates, InputStream bksFile,
      String password) {
    try {
