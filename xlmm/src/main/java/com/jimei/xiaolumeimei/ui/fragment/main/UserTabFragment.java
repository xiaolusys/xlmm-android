...
import com.jimei.xiaolumeimei.databinding.FragmentUserTabBinding;
...
public class UserTabFragment extends BaseBindingFragment<FragmentUserTabBinding> implements View.OnClickListener {
    ...
    private void setIcon(int id, ImageView imageView) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            drawable = mActivity.getDrawable(id);
        } else {
    ...
    private void fillDataToView(UserInfoBean userInfoBean) {
        if (userInfoBean != null) {
            ...
            if (mamaFlag) {
                ...
                UdeskSDKManager.getInstance().setUserInfo(mActivity, id, info);
                getMamaData();
    ...
    private void getMamaData() {
        UdeskSDKManager.getInstance().initApiKey(mActivity, XlmmConst.UDESK_URL, XlmmConst.UDESK_KEY);
        VipInteractor vipInteractor = XlmmApp.getVipInteractor(mActivity);
                    ...
                    addSubscription(XlmmApp.getVipInteractor(mActivity)
                        .getWxCode(new ServiceResponse<WxQrcode>() {
                            ...
                            public void onNext(WxQrcode wxQrcode) {
                                ...
                                readyGo(MMNinePicActivity.class, bundle);
                            }
                        }));
                    break;
                case R.id.ll_choose:
