...
public class CartsPayInfoActivity extends BaseSwipeBackCompatActivity
    ...
    @Bind(R.id.common_icon_right_arrow)
    ImageView commonIconRightArrow;
    @Bind(R.id.scb)
    SmoothCheckBox scb;
    @Bind(R.id.tv_alipay)
    TextView tvAlipay;
    ...
    View v2;
    @Bind(R.id.alipay)
    AppCompatRadioButton alipay;
    @Bind(R.id.wx)
    AppCompatRadioButton wx;
    @Bind(R.id.tv2)
    TextView tv2;
    @Bind(R.id.tv1)
    TextView tv1;
    ...
    private CartsPayinfoBean.payExtrasEntityApp payExtrasEntityApp;
    private CartsPayinfoBean.payExtrasEntityApp payExtrasEntityAppBudget;
            ...
            case R.id.confirm:
                if (isHaveAddress) {
                    if (isCoupon) {
                        ...
                        if (isAlipay) {
                            pay("alipay", pay_extras);
                        }

                        if (isBudget) {

                            if (payExtrasEntityAppBudget.getValue() > paymentInfo) {
                                pay("isBudget", pay_extras);
                            } else {
                                Toast.makeText(this, "余额不足,请选择下面一种支付方式一起使用", Toast.LENGTH_SHORT).show();

                            }

                        }

                        if (isBudget && isAlipay) {
                            pay("alipay", pay_extras_budget);
                        }
    ...
    private void pay(String pay_method, String pay_extrasaa) {
                    ...
                    public void onNext(PayInfoBean payInfoBean) {
                        ...
                        if (null != payInfoBean) {
                            ...
                            if (isBudget) {
                                ...
                                intent.putExtra(PaymentActivity.EXTRA_CHARGE, payInfoBean.getCharge());
