package com.jimei.xiaolumeimei.widget.timecircleview;
...
import com.jimei.xiaolumeimei.widget.timecircleview.utils.GeometryUtil;
...
public class TickCircleProgress extends CircleProgress {
  ...
  @Override protected void initViewSize() {
    super.initViewSize();

    float dx = mStrokeWidth * 0.5F;
    mParentRect =
        new RectF(mPaintRectF.left + dx, mPaintRectF.top + dx, mPaintRectF.right - dx,
            mPaintRectF.bottom - dx);

    mTickPaintRect =
        new RectF(mParentRect.left + tickMarkHeight, mParentRect.top + tickMarkHeight,
            mParentRect.right - tickMarkHeight, mParentRect.bottom - tickMarkHeight);

    startPoints = GeometryUtil.getIntersectionPoints(mCenter,
        (mParentRect.right - mParentRect.left) / 2F, 0.0);
    stopPoints = GeometryUtil.getIntersectionPoints(mCenter,
        (mTickPaintRect.right - mTickPaintRect.left) / 2F, 0.0);
  }

  @Override protected void initVar(Context context, AttributeSet attrs) {
    ...
    tickMarkHeight =
        ta.getDimension(R.styleable.TickCircleProgress_tickMarkHeight, mStrokeWidth * 1F);
    ta.recycle();
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < tickMarkCount; i++) {
      if (i != 0) canvas.rotate(-routeDegress, mCenter.x, mCenter.y);
      canvas.drawLine(startPoints[0].x, startPoints[0].y, stopPoints[0].x,
          stopPoints[0].y, tickPaint);
    }
  }
}
