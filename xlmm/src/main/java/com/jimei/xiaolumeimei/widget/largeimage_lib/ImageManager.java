package com.jimei.xiaolumeimei.widget.largeimage_lib;
...
public class ImageManager {
  ...
  @TargetApi(Build.VERSION_CODES.KITKAT) public static int getBitmapSize(Bitmap bitmap) {
  ...
  public List<DrawData> getDrawData(float imageScale, Rect imageRect) {
    ...
    if (cacheImageData == null) {
      try {
        ...
        int s = (int) Math.sqrt(
            imageWidth * imageHeight / (screenWidth / 2) / (screenHeight / 2));
    ...
    int startRow =
        imageRect.top / blockSize + (imageRect.top % blockSize == 0 ? 0 : 1) - 1;
    int endRow =
        imageRect.bottom / blockSize + (imageRect.bottom % blockSize == 0 ? 0 : 1);
    int startCol =
        imageRect.left / blockSize + (imageRect.left % blockSize == 0 ? 0 : 1) - 1;
    ...
    if (loadData.mCurrentCacheData == null) {
      loadData.mCurrentCacheData =
          new CacheData(scale, new ConcurrentHashMap<Position, Bitmap>());
    ...
    if (!needShowPositions.isEmpty()) {
      ...
      while (iterator.hasNext()) {
        ...
        if (scaleKey / scale == 2) {// 这里都是大于scale的.,缩小的图，单位区域大
            ...
            for (int row = startPositionRow, i = 0; row <= endPositionRow; row++, i++) {
              ...
              for (int col = startPositionCol, j = 0; col <= endPositionCol; col++, j++) {
                ...
                if (needShowPositions.remove(new Position(row, col))) {
                  ...
                  drawDatas.add(
                      new DrawData(bitmap, new Rect(left, top, right, bottom), rect));
                }
              }
            }
          }
          ...
          int startRowKey =
              imageRect.top / size + (imageRect.top % size == 0 ? 0 : 1) - 1;
          int endRowKey =
              imageRect.bottom / size + (imageRect.bottom % size == 0 ? 0 : 1);
          int startColKey =
              imageRect.left / size + (imageRect.left % size == 0 ? 0 : 1) - 1;
          int endColKey = imageRect.right / size + (imageRect.right % size == 0 ? 0 : 1);

          Log.d("nnnn", "startRowKey"
              + startRowKey
              + " endRowKey:+"
              + endRowKey
              + " endColKey:"
  ...
  private static class InputStreamBitmapRegionDecoderFactory
    ...
    @Override public BitmapRegionDecoder made() throws IOException {
      return BitmapRegionDecoder.newInstance(inputStream, false);
    }
  }

  private static class PathBitmapRegionDecoderFactory
      implements BitmapRegionDecoderFactory {
    private String path;
  ...
  private class NearComparator implements Comparator<CacheData> {
    ...
    @Override public int compare(CacheData lhs, CacheData rhs) {
