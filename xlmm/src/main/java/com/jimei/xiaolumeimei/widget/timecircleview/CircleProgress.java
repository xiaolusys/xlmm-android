...
public class CircleProgress extends View {
  ...
  protected void initVar(Context context, AttributeSet attrs) {
    ...
    progressInitialPosition =
        ta.getInteger(R.styleable.CircleProgress_progressInitialPosition, 135);
    ...
    currProgress =
        currProgress <= maxProgress && currProgress >= 0 ? currProgress : maxProgress;
  }
  ...
  protected void initViewSize() {
    ...
    mPaintRectF = new RectF(0 + mStrokeWidth, 0 + mStrokeWidth, mWidth - mStrokeWidth,
        mHeight - mStrokeWidth);

    if (useGradient) {
      LinearGradient gradient =
          new LinearGradient(0, 0, mWidth, mHeight, foreEndColcor, foreStartColor,
  ...
  protected void valueAnimator(final float originProgress, final float endProgress) {
    ...
    mValueAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
      @Override public void onAnimationUpdate(ValueAnimator mAnim) {
    ...
    int duration = (int) (ANIMATION_DURATION * (Math.abs((endProgress - originProgress))
        / maxProgress));
  ...
  public void setProgress(int progress) {
    if (progress < 0 || progress > 100) {
      throw new IllegalArgumentException(
          "progress must >=0 && <=100，now progress is " + progress);
    }
